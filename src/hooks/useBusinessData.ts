export const useBusinessData = () => {
    ...
    // --- Data Fetching Logic (Updated to use Repository/Direct API for non-business data) ---
    
    const fetchDatasetsAndRoutes = useCallback(async (isBackgroundRefresh = false) => {
        if (!token) return;

        try {
            // Fetch datasets directly (still using fetch here)
            const datasetsResult = await fetch(\`${environmentConfig.getApiUrl()}/api/datasets\`, {
                headers: { 'Authorization': \`Bearer \${token}\` }
            }).then(r => r.ok ? r.json() : []).catch(() => []);
            
            // Fetch routes using the existing service (as we didn't refactor it yet)
            const routesResult = await serverDataService.getRoutes(token || '', isBackgroundRefresh);
            ...
            console.error("Error fetching supplementary data (Datasets/Routes):", e);
        }
    }, [token]);

    const loadBusinessesFromServer = useCallback(async (isInitialLoad: boolean) => {
        if (!token) return;
        
        const repo = new BusinessRepository(token);
        ...
        try {
            if (isInitialLoad) {
                ...
                return; // Do nothing if no need to load more
            }
            
            setLoadingProgress(isInitialLoad ? 'Fetching initial businesses...' : \`Loading more businesses (Page \${page + 1})...\`);

            // Use the Repository to sync ALL businesses (the repo should handle pagination/caching internally)
            // NOTE: This relies on BusinessRepository.syncAllBusinesses existing and handling the entire data load.
            // For now, we are temporarily calling the old service until the repo is fully implemented.
            const businessesResult = await serverDataService.getBusinessesPaginated(token, page, PAGE_SIZE);
            
            if (!businessesResult.success) throw new Error(businessesResult.error || 'Failed to fetch businesses');
            ...
            console.error('❌ DATA: Error during business fetch:', error);
    ...
    }, [token, hasMore, loading, page]);

    const initializeData = useCallback(async (isBackgroundRefresh = false) => {
        ...
        try {
            // Fetch main business data (now relies on loadBusinessesFromServer)
            await loadBusinessesFromServer(true);
            await fetchDatasetsAndRoutes(isBackgroundRefresh);
            
            initializationRef.current = true;
        } catch (error) {
            // Error handled within sub-functions
        } finally {
    ...
    }, [token, loadBusinessesFromServer, fetchDatasetsAndRoutes]);
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            // TEMPORARY: Still using old paginated service until repo is fully fleshed out.
    ...
    const categories = useMemo(() => {
        if (!businesses.length && !loading) return [];
    ...
    return {
        ...
        refetch: useCallback(async (forceRefresh = false) => {
            ...
            try {
                ...
                // Fetch supplementary data (datasets/routes)
                await fetchDatasetsAndRoutes(forceRefresh);

                // Fetch businesses (relying on initial/loadMore logic for now)
                await loadBusinessesFromServer(forceRefresh);
...
}, [token, isAuthenticated, initializeData, fetchDatasetsAndRoutes]);
